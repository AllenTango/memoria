...
Memoria 测试目录，采用隔离fixture模式，所有测试在临时目录执行，不污染源码。
...
├── README.md              # 本文件
├── unit/                  # 单元测试（vitest）
│   └── cli.test.ts        # CLI 命令单元测试
├── integration/           # 集成测试（完整 CLI 流程）
│   └── memoria-integration-test.ts  # 主力集成测试脚本
├── mock/                  # TUI 模拟测试用的 mock 数据
├── scripts/               # 测试辅助脚本
...
```bash
npx tsx tests/integration/memoria-integration-test.ts
```
...
- 如需跳过某个测试项，可临时修改 `integration/memoria-integration-test.ts`
